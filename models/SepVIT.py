...
class DSSA(nn.Module):
    def __init__(
        ...
        dim,
        heads = 8,
        #dim_head = 32, Removed by Saeid
        dropout = 0.,
        window_size = 7
    ):
        super().__init__()
        self.heads = heads
        # self.scale = dim_head ** -0.5  # Saeid: I removed predefiend and fixed dim_head and redefine it based on Swin
        dim_head= dim // heads 
        self.scale=  dim_head ** -0.5 
        self.window_size = window_size
        inner_dim = dim_head * heads # In Swin inner_dim= 3*dim
    ...
    def forward(self, x):
        ...
        #print(f"batch: {batch}\theight:{height}\twidth:{width}\theads:{heads}\twsz:{wsz}")
...
class Transformer(nn.Module):
    def __init__(
        ...
        dim,
        depth,
       # dim_head = 8, # Removed by Saeid in order to have self.scale as defined in Swin
        heads = 8, # 
        window_size=4,
        ff_mult = 4,
        dropout = 0.,
        ...
        for ind in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(dim, DSSA(dim, heads = heads, dropout = dropout,window_size=window_size)), # attention part
                PreNorm(dim, FeedForward(dim, mult = ff_mult, dropout = dropout)), # MLP part
            ]))
...
class SepViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        #strides = (4, *((2,) * (num_stages - 1))) # Original Code
        strides = (1, *((2,) * (num_stages - 1))) # Saeid: I changed 4 into 1 because the input feature map size is not 224
        # Changed by Saeid
        ...
        for ind, ((layer_dim_in, layer_dim), layer_depth, layer_stride, layer_heads, layer_window_size) in enumerate(zip(dim_pairs, depth, strides, heads,window_sizes )):
            is_last = ind == (num_stages - 1)
            # print('****************************************************')
            # print (f'\nlayer_dim_in:{layer_dim_in}  layer_dim:{layer_dim}   layer_depth:{layer_depth}\n')
            # print(f'layer_stride:{layer_stride}   layer_heads:{layer_heads}   layer_window_size:{layer_window_size}\n')
            # print('****************************************************\n')

            self.layers.append(nn.ModuleList([
                OverlappingPatchEmbed(layer_dim_in, layer_dim, stride = layer_stride),
                PEG(layer_dim),
                Transformer(dim = layer_dim, depth = layer_depth, heads = layer_heads,window_size=window_size,
                            ff_mult = ff_mult, dropout = dropout, norm_output = not is_last),
            ]))
