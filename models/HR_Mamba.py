...
class HighResolutionNet(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.project_low = nn.Conv2d(32, 224, kernel_size=1, bias=False)
        self.project_mid = nn.Conv2d(64, 224, kernel_size=1, bias=False)
        self.project_mid2 = nn.Conv2d(224, 224, kernel_size=1, bias=False)  # ASPP output is 256
        self.project_high = nn.Conv2d(256, 224, kernel_size=1, bias=False) # ASPP output is 256

        # Final fusion and classifier
        self.fusion_conv = nn.Sequential(
            nn.Conv2d(224 * 3, 256, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(256),
        ...
        self.fusion_conv1 = nn.Sequential(
            nn.Conv2d(224 * 2, 224, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(224),
            #LayerNorm(256, data_format="channels_first"),
            nn.GELU(),
        )
    ...
    def _make_head(self, pre_stage_channels):
        ...
        head_channels = [32, 64, 224, 256]

        # Increasing the #channels on each resolution
        # from C, 2C, 4C, 8C to 224, 256, 512, 1024
        incre_modules = []
...
class PositionalEncoding2D(nn.Module):
    def __init__(self, channels, height=224, width=224):
        """
        完全修正的2D位置编码
...
class MLPSegmentationWithPE(nn.Module):
    def __init__(self, input_channels=10, num_classes=5, hidden_dim=512, pe_dim=32):
        ...
        self.flattened_dim = 224 * 224 * (input_channels + pe_dim)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.LayerNorm(hidden_dim),
            nn.Linear(hidden_dim, 224 * 224 * num_classes)
        )

    def forward(self, x):
        bs = x.shape[0]

        # 生成位置编码 [bs, pe_dim, 224, 224]
        pe = self.pos_encoder(torch.zeros(bs, self.pe_dim, 224, 224, device=x.device))

        # 拼接输入和位置编码 [bs, 10+pe_dim, 224, 224]
        x_with_pe = torch.cat([x, pe], dim=1)

        # 展平输入 [bs, (10+pe_dim)*224*224]
        x_flat = x_with_pe.view(bs, -1)

        # 通过MLP [bs, 224*224*num_classes]
        out_flat = self.mlp(x_flat)

        # 重塑为 [bs, num_classes, 224, 224]
        out = out_flat.view(bs, self.num_classes, 224, 224)
...
if __name__ == '__main__':
    ...
    input_tensor = torch.randn(1, 10, 224, 224).to(device=device)
