...
class CPVTV2(PyramidVisionTransformer):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.num_features=embed_dims[-1]# Added by Saeid
        self.num_classes=num_classes# Added by Saeid
        self.F4 = F4
        ...
        self.head = nn.Linear(self.num_features, self.num_classes, bias=True)
        self.apply(self._init_weights)
    ...
    def forward_features(self, x):
        ...
        for i in range(len(self.depths)):
            ...
            outputs.append(x) # It is a list icludes output of each stage which for classficationonly tha last one is needed
            
        # For having the last stage's output, F4 must be set as True
        #outputs=outputs[-1] # Output of the last stage
        
        
        #self.head=self.head.to(torch.device("cuda:0")
    ...
    def forward(self, x):
        ...
        #Added by Saeid
        #x=x[-1]# Convert a list into a torch
        B,C,h,w=x.shape
...
import torch
Input=torch.rand((2,3,Image_Sizes[idx-1],Image_Sizes[idx-1]))

Twins_model_config = { "img_size":Image_Sizes[idx-1], 
                      "patch_size":1, 
                      "in_chans":3,
                      "num_classes":6,
                      "embed_dims":Embed_Dims[0:-1] if idx !=3 else Embed_Dims, # [64, 128, 320, 512], 
                      "num_heads":Num_Heads[0:-1] if idx != 3 else Num_Heads, 
                      "mlp_ratios":[1*MLP_Ratio for i in range (len(Depths[0:-1]) if idx != 3 else len(Depths) )],
                      "qkv_bias":True,  
                      "depths":Depths[0:-1] if idx != 3 else Depths, 
                      "sr_ratios":sr_ratio[0:-1] if idx !=3 else sr_ratio,
                      "drop_rate":0.0, 
                      "drop_path_rate":0.2,
                      "F4":True} # F4 must be True for classification 


Model=CPVTV2(**Twins_model_config)
...
class pcpvt_small_v0(CPVTV2):
    def __init__(self, **kwargs):
        super(pcpvt_small_v0, self).__init__(img_size=56,num_classes=2,
            patch_size=1, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.2,F4=True)
