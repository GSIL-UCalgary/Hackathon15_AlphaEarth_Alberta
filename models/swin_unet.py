...
class PatchEmbed(nn.Module):
    r""" Image to Patch Embedding to increase feature number and reduce the spatial dimensions
    Args:
    ...
    def __init__(self, img_size=128, patch_size=1, in_chans=10, embed_dim=32, norm_layer=None):
        super().__init__()
...
class SwinUNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        depths_decoder = common['depths']
        in_chans = common ['in_channels']
        self.num_classes = common ['num_classes']
        num_heads = common['heads']
        self.stem_ds= common['stem_downsampling']

        swin           = common['swin']
        window_size = swin['window_size']
        self.patch_size     = swin['patch_size']
        self.drop_rate      = swin['drop_rate']
        self.embed_dim      = swin['embed_dim']
        self.mlp_ratio      = swin['mlp_ratio']
        self.qkv_bias       = swin['qkv_bias']
        self.qk_scale       = swin['qk_scale']
        self.attn_drop_rate = swin['attn_drop_rate']
        self.drop_path_rate = swin['drop_path_rate']
        self.ape            = swin['ape']
        self.patch_norm     = swin['patch_norm']
        use_checkpoint = swin['use_checkpoint']
        self.final_upsample = swin['final_upsample']
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size= self.patch_size, in_chans=in_chans, embed_dim= self.embed_dim,
        ...
        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, self.drop_path_rate, sum(depths))]  # stochastic depth decay rule
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(self.embed_dim * 2 ** i_layer),
                               ...
                               num_heads= num_heads, #num_heads[i_layer],
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                ...
                layer_up = BasicLayer_up(dim=int(self.embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                         ...
                                         num_heads= num_heads, #num_heads[(self.num_layers - 1 - i_layer)],
                                         window_size=window_size,
                                         mlp_ratio=self.mlp_ratio,
                                         qkv_bias=self.qkv_bias, qk_scale=self.qk_scale,
                                         drop=self.drop_rate, attn_drop=self.attn_drop_rate,
        ...
        # always need a final Conv2d to go from embed_dim → num_classes
        self.output = nn.Conv2d(in_channels=self.embed_dim, out_channels=self.num_classes, kernel_size=1, bias=False)

        # only build the learned patch‐expander when you did actually pool in the stem
        if self.stem_ds:
            print("→ using FinalPatchExpand_X2 to upsample back to full resolution")
            # input_resolution is the number of patches per side after stem (img_size//patch_size)
            pe_side = img_size // self.patch_size
            self.up = FinalPatchExpand_X4(
                input_resolution=(pe_side, pe_side),
                dim=self.embed_dim,
                dim_scale=2,          # total down‐sampling factor = 4 (2 from stem + 2 from decoder)
                norm_layer=nn.LayerNorm
            )
        else:
            # no spatial shrinkage, so no need to expand
            self.up = nn.Identity()
    ...
    def forward(self, x):
        ...
        #print(f"x_downsample after self.forward_features(x): {x_downsample}")
        x = self.forward_up_features(x, x_downsample)
        # reshape x from B, H*W, C into 
        x = self.up_x4(x)
        return x
...
if __name__=='__main__':
    ...
    summary(model, input_size=(1, 10, 128, 128))