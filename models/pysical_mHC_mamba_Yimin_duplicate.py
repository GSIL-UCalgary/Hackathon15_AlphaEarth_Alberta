...
class ImageHyperConnectionBlock(nn.Module):
    ...
    def forward(self, h: torch.Tensor, mask=None) -> torch.Tensor:
        ...
        # -----------------------------
        H, W = 224, 224  # You might want to make this configurable
        ...
        # Collect indices for all batches
        for cluster_idx in range(cluster_num):
            cluster_scores = clustering[:, :, cluster_idx]  # [B, HW]
            _, topk_indices = torch.topk(cluster_scores, k=k, dim=-1)  # [B, k]
        ...
        # Process each cluster with proper batch handling
        x_processed = []
        for i in range(cluster_num):
            # Get indices for this cluster [B, k]
            indices = selected_indices[i]  # [B, k]
            
            # Process each batch separately or use batched gather
            cluster_output = []
            for batch_idx in range(B):
                batch_indices = indices[batch_idx]  # [k]
                batch_data = attn_output[batch_idx, batch_indices, :]  # [k, C]
                processed = self.spatial_adpative_clustering_branch[i](batch_data.unsqueeze(0))  # [1, k, C]
                cluster_output.append(processed.squeeze(0))
            
            # Stack batch results
            x_processed.append(torch.stack(cluster_output))  # [B, k, C]
        
        # Scatter add with proper batch handling
        for i in range(cluster_num):
            indices = selected_indices[i]  # [B, k]
            
            # Expand indices for scattering
            expanded_indices = indices.unsqueeze(-1).expand(-1, -1, C)  # [B, k, C]
            
            # Scatter for each batch
            for batch_idx in range(B):
                output[batch_idx].scatter_add_(
                    0,  # dimension to scatter along
                    expanded_indices[batch_idx],  # [k, C]
                    x_processed[i][batch_idx]  # [k, C]
                )
...
class ImageHyperConnectionTransformer_mhc_replicate(nn.Module):
    def __init__(self, image_size: int, patch_size: int, in_channels: int, num_classes: int, 
                 ...
                 pool_size=4, mask_ratio=0.1, dynamic=True, sensor_name='alphaearth'):
        super().__init__()
        if dim % 2 != 0:
            raise ValueError(f"dim must be even for SinusoidalPositionEncoding, got {dim}")
        ...
        self.dim = dim
        if sensor_name == 'landsat8':
            vis_idx = [0,1,2]
            nir_idx = [3]
            swir1_idx = [4]
            swir2_idx = [5]
            full_idx = [0,1,2,3,4,5]
        elif sensor_name == 'sentinel2':
            vis_idx = [0,1,2]
            nir_idx = [3, 4, 5, 6, 7]
            swir1_idx = [8]
            swir2_idx = [9]
            full_idx = list(range(0,10))
        elif sensor_name == 'alphaearth':
            vis_idx= list(range(0,16))
            nir_idx= list(range(16,32))
            swir1_idx= list(range(32,48))
            swir2_idx= list(range(48,64))
            full_idx= list(range(0,64))
        else:
            raise ValueError("Error: The name of the sensor is not correct.\nIt must be between ['landsat8', 'sentinel2', 'alphaearth']")
        ...
        #     "SWIR2": swir2_idx,
        # }
        self.expansion_rate = len(self.band_groups)-1  # = 5
        ...
        self.pos_embed = nn.Parameter(pos_embed)  # Now it's learnable!
        ...
        self.upsampling = nn.Upsample(scale_factor=self.patch_size, mode='bilinear', align_corners=False)
    ...
    def forward(self, x: torch.Tensor, gt_Map=None, epoch=None, return_features=False):
        ...
        x = self.patch_embed(x).flatten(2).transpose(1, 2)    # B 96, 8, 8. --> B, 96, 64
...
if __name__ == '__main__':
    ...
    # --- 图像模型测试 ---
    img_model = ImageHyperConnectionTransformer_mhc(
